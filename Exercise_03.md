...
We will get our feet wet with writing a JavaScript check to explore the interface, then we will implement a basic Java 
check, and then we'll implement a configurable Java `ProgessCheckFactory` when we are ready to consider reuse.
...
`<impl>myFirstCheck.js</impl>` element under that. It should now look like this:
